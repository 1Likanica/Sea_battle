...
# Обозначение фигур
class Cell(object): 
  empty_cell = set_color('◌', Color.void)
      ...
      for x in range(-1, self.size):
        for y in range(-1, self.size):
          ...
          if x >= 0 and y == -1:
            ...
            continue
          print(" " + str(field[x][y]), end='') #Можно кншн добавить '|', но будет не красиво
        print("")
    print("")
...
# В классе Game мы опишем буквы используемые для отображения координат.
class Game(object):
  letters = ("A", "B", "C", "D", "E", "F", "G")
  ships_rules = [1, 1, 1, 1, 2, 3]
...
if __name__ == '__main__':
  ...
  players = []
  players.append(Player(name='Likanica', is_ai=False, auto_ship=True, skill=1))
  players.append(Player(name='Master_Bot', is_ai=True, auto_ship=True, skill=1))
